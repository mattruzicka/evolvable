module Evolvable
    ...
    def call!(individuals)
      ...
      mutant_genes = generate_mutant_genes_array(population_mutation_count)
    ...
    def generate_mutant_genes_array(mutation_count)
      gene_pool_size = evolvable_gene_pool.size
      mutant_genes = []
      while mutant_genes.count < mutation_count
        genes_count = [gene_pool_size, mutation_count - mutant_genes.count].min
        mutant_genes.concat Evolvable.random_genes(@evolvable_class, genes_count).to_a
      end
      mutant_genes
    end
	end
